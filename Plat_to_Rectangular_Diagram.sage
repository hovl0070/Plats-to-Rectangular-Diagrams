def Plat_to_Rectangular_Diagram(W):
    #INPUT: the braid word whose plat closure is a knot.  
    #For now, the plat must be a knot and not a link. If a link is the input we can still obtain the vertex set of the rectangular diagram but to plot it requires orienting the rectangular diagram with the rectangulardiagram_to_orientedrectangulardiagram function which does not currently work to orient a link. 
...
def syllable_string_partitioning(array):
    ...
    # Break into beginning and end of syllables for plat to rectangular diagram input
...
def rectangulardiagram_to_orientedrectangulardiagram(V):
    #The input into this must be a vertex set corresponding to a knot.  If it is a link it will not work yet.  This can be fixed in the future.
